...
impl crate::schemars::JsonSchema for DaemonSetSpec {
    fn schema_name() -> String {
        "io.k8s.api.apps.v1.DaemonSetSpec".to_owned()
    }
                    ...
                    (
                        "selector".to_owned(),
                        {
                            ...
                            schema_obj.metadata = Some(Box::new(crate::schemars::schema::Metadata {
                                description: Some("A label query over pods that are managed by the daemon set. Must match in order to be controlled. It must match the pod template's labels. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors".to_owned()),
                                ..Default::default()
                            }));
